...
5. Discordの埋め込みメッセージを使用して表示をするので、それにあわせて回答をフォーマットしてください

【重要：長い回答を分割するための指示】
もし回答が長くなる場合は、以下の規則に従ってください：
1. 約4000文字ごとに必ず "===SPLIT_MESSAGE===" というマーカーを挿入してください
2. このマーカーは必ず段落の区切りに挿入してください
3. コードブロック内にマーカーを挿入しないでください - コードブロックの前後に配置してください
4. マーカーを挿入する前後には必ず改行を入れてください
5. マーカーが入っている箇所でメッセージが分割され、別々のメッセージとして送信されます
`,
} as const;
