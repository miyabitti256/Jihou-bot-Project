...
export async function fetchGuild(guildId: string) {
  ...
  if (!response.ok) {
    logger.error(
      `[discord-api] Failed to fetch guild ${guildId}: ${response.status}`,
    );
    throw new DiscordApiError(
      "GUILD_NOT_FOUND",
...
export async function fetchGuildChannels(guildId: string) {
  ...
  if (!response.ok) {
    logger.error(
      `[discord-api] Failed to fetch channels for guild ${guildId}: ${response.status}`,
    );
    throw new DiscordApiError(
      "CHANNELS_NOT_FOUND",
...
export async function fetchDiscordUser(userId: string) {
  ...
  if (!response.ok) {
    logger.error(
      `[discord-api] Failed to fetch user ${userId}: ${response.status}`,
    );
    throw new DiscordApiError(
      "USER_NOT_FOUND",
...
export async function fetchChannel(channelId: string) {
  ...
  if (!response.ok) {
    logger.error(
      `[discord-api] Failed to fetch channel ${channelId}: ${response.status}`,
    );
    throw new DiscordApiError(
      "CHANNEL_NOT_FOUND",
